...
def affine_forward(x, w, b):
    ...
    out = x.reshape(batch_size, -1).dot(w) +  b  # (N, D)
...
def affine_backward(dout, cache):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    batch_size = x.shape[0]
    dw = x.reshape(batch_size, -1).T.dot(dout)  # (D, N) X (N, M)
    dx = dout.dot(w.T).reshape(batch_size, -1).reshape(x.shape)  # (N, M) X (M, D)
    db = dout.sum(axis=0)  # (N, M)
...
def svm_loss(x, y):
    ...
    correct_score = x[row_idx, y].reshape(-1, 1)

    # compute the multiclass svm loss
    loss = x - correct_score + 1
    loss = np.maximum(0, loss)
    loss[row_idx, y] = 0

    # compute the gradient
    dx = (loss > 0).astype(float)
    dx[row_idx, y] -= dx.sum(axis=1)
    dx /= batch_size

    loss = loss.sum(axis=1).mean(axis=0)
...
def softmax_loss(x, y):
    ...
    loss = -np.log(yhat[row_idx, y]).mean()
